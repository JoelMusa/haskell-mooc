...
isums :: Int -> IO Int
isums n = isumsHelper n 0 0

isumsHelper :: Int -> Int -> Int -> IO Int
isumsHelper 0 _ totalSum = return totalSum
isumsHelper n currentSum totalSum = do
  input <- getLine
  let number = read input :: Int
  let newSum = currentSum + number
  print newSum
  isumsHelper (n - 1) newSum (totalSum + number)
