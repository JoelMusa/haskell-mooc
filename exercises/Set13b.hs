...
mapM2 op xs ys = mapM2Helper op xs ys []

mapM2Helper :: Monad m => (a -> b -> m c) -> [a] -> [b] -> [c] -> m [c]
mapM2Helper _ [] _ acc = return (reverse acc)  -- Base case: reached end of first list
mapM2Helper _ _ [] acc = return (reverse acc)  -- Base case: reached end of second list
mapM2Helper op (x:xs) (y:ys) acc = do
    result <- op x y  -- Apply op to current elements
    mapM2Helper op xs ys (result : acc)  -- Recur with the result appended to accumulator
...
path maze place1 place2 = place2 `elem` reachablePlaces
  where
    (_, reachablePlaces) = runState (visit maze place1) []
